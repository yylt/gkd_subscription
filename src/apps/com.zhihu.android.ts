      ...
      rules: [
        {
          activityIds: [
            ...
            '.mix.activity.ContentMixProfileActivity',
            '.app.ui.activity.HostActivity',
            '.login.ui.activity.LoginActivity',
            '.account.LoginActivity',
          ],
          matches: [
            '[text*="登录" || text*="注册" || text*="登入" || text*="账号登录"][visibleToUser=true]',
            '[text*="关闭" || text*="取消" || text*="跳过" || text*="暂不" || text*="以后再说" || vid="close" || vid="iv_close" || vid="btn_close"][visibleToUser=true]',
          ],
        },
      ],
    },
